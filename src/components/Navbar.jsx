...
import { setToggleLightMode, setToggleDarkMode } from './../app/ui';
...
function Navbar() {
  ...
  return (
    <nav className={` ${invoiceForm ? 'absolute w-full lg:sticky lg:w-28' : "lg:w-28 sticky"} bg-custom-dark-blue-200  h-20 lg:h-screen  flex lg:flex-col justify-between lg:rounded-tr-xl lg:rounded-br-xl  left-0 top-0 z-50`
} >
      ...
      <aside className='flex lg:flex-col gap-6 '>
        {
          theme === "ligth"
            ?
          <img  onClick={() => dispatch(setToggleDarkMode())} className='w-6 h-6 self-center cursor-pointer' src={moonSvg} alt="" /> :
            <img onClick={() => dispatch(setToggleLightMode())} className='w-6 h-6 self-center cursor-pointer' src={sunSvg} alt="" />
        }
